...
- `tests/managed_agent_definitions.test.js` verifies the Managed Agents system prompt keeps the reminder-default policy needed by parity stories, routes submitted/ungraded questions through the icon-aware filter, enables only `web_search`/`web_fetch` plus memory file tools from the built-in agent toolset, keeps `bash` disabled, and includes memory guardrails against storing secrets, raw grades, full assignment lists, private student records, unsafe content, or verbatim fetched/web content.
